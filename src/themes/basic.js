...
import MaterialIcons from './MaterialIcons.ttf';
...
export const basic = {
  ...
  header:{
    textColor:"white",
    position:"relative",
    color:function(){return basic.color.primary},
    height:60,
    elevation:"none"
  },
  footer:{
    ...
    item:{
      ...
      transition:function(vertical ,left,go){
        ...
        if (vertical) {
          ...
          if (left) {
            margin={
              from:`-${basic.menu.item.margin.vertical.out}`,
              to:`-${basic.menu.item.margin.vertical.in}`
            }
          }else {
            margin={
              from:basic.menu.item.margin.vertical.out,
          ...
          direction="top";
          margin={
            from:basic.menu.item.margin.horizontal.out,
            to:basic.menu.item.margin.horizontal.in
          }
        }
        let opacity ={
          from:0,
